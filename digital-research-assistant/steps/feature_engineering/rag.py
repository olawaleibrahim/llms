...
from digital_research_assistant.application.preprocessing import ChunkingDispatcher, EmbeddingDispatcher
from digital_research_assistant.domain.chunks import Chunk
...
def chunk_and_embed(
    ...
    for document in cleaned_documents:
        ...
        metadata["chunking"] = _add_chunks_metadata(
            chunks, metadata["chunking"])

        for batched_chunks in utils.misc.batch(chunks, 10):
            batched_embedded_chunks = EmbeddingDispatcher.dispatch(
                batched_chunks)
...
def _add_chunks_metadata(chunks: list[Chunk], metadata: dict) -> dict:
    for chunk in chunks:
        ...
        metadata[category]["num_chunks"] = metadata[category].get(
            "num_chunks", 0) + 1
