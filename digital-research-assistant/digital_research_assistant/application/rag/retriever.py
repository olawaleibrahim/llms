...
class ContextRetriever:
    ...
    def search(
        ...
        if len(n_k_documents) > 0:
            k_documents = self.rerank(
                query, chunks=n_k_documents, keep_top_k=k)
        else:
            k_documents = []
    ...
    def _search(self, query: Query, k: int = 3) -> list[EmbeddedChunk]:
        ...
        articles_chunks = _search_data_category(
            EmbeddedArticleChunk, embedded_query)
    ...
    def rerank(self, query: str | Query, chunks: list[EmbeddedChunk], keep_top_k: int) -> list[EmbeddedChunk]:
        ...
        logger.info(
            f"{len(reranked_documents)} documents reranked successfully.")
