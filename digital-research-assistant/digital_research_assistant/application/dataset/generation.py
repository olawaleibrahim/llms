...
from digital_research_assistant import domain
...
from digital_research_assistant.domain.dataset import DatasetType, TrainTestSplit
from digital_research_assistant.domain.prompt import GenerateDatasetSamplesPrompt, Prompt
...
"""
    ...
    def get_prompts(cls, documents: list[CleanedDocument]) -> dict[DataCategory, list[GenerateDatasetSamplesPrompt]]:
        ...
        grouped_prompts = {}
        grouped_cleaned_documents = CleanedDocument.group_by_category(
            documents)
        for category, category_documents in grouped_cleaned_documents.items():
            category_prompts = [cls.get_prompt(
                document) for document in category_documents]
    ...
    def generate(
        ...
        for category, category_prompts in prompts.items():
            langchain_category_prompts = [_to_langchain(
                prompt) for prompt in category_prompts]
...
"""
    ...
    def post_process_datasets(
        ...
        remaining_samples = sum(
            [dataset.num_samples for dataset in datasets.values()])
        logger.info(
