...
from digital_research_assistant.application.preprocessing.operations.chunking import chunk_document
from digital_research_assistant.domain.cleaned_documents import CleanedDocument
from digital_research_assistant.domain.dataset import (
    InstructDataset,
...
def create_instruct_train_test_split(
    ...
    for category, dataset in data.items():
        ...
        if len(samples_dicts) > 0:
            ...
            train_samples = [InstructDatasetSample(
                **sample_dict) for sample_dict in train_samples_dicts]
            test_samples = [InstructDatasetSample(
                **sample_dict) for sample_dict in test_samples_dicts]
        else:
            train_samples = []
            test_samples = []
...
def create_preference_train_test_split(
    ...
    for category, dataset in data.items():
        ...
        if len(samples_dicts) > 0:
            ...
            train_samples = [PreferenceDatasetSample(
                **sample_dict) for sample_dict in train_samples_dicts]
            test_samples = [PreferenceDatasetSample(
                **sample_dict) for sample_dict in test_samples_dicts]
        else:
            train_samples = []
            test_samples = []
...
def filter_short_answers(
    ...
    filtered_data = {}
    for category, dataset in data.items():
        filetered_dataset_samples = list(
            filter(is_long_enough, dataset.samples))
...
def filter_answer_format(data: dict[DataCategory, PreferenceDataset]) -> dict[DataCategory, PreferenceDataset]:
    ...
    for category, dataset in data.items():
        filetered_dataset_samples = list(
            filter(is_valid_format, dataset.samples))
        filtered_dataset = PreferenceDataset(
            category=category, samples=filetered_dataset_samples)
...
def extract_substrings(
    ...
    for document in documents:
        document_extracts = chunk_document(
            document.content, min_length, max_length)
